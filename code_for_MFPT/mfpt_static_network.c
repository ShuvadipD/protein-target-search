...
/*Use double , you have floating numbers not int*/
	...
	FILE *file1;
	file1=fopen("raw_exit_times_power_law_static_500.dat", "w");
  

	FILE *file2;
	file2=fopen("meanfpt_power_law_static_500.dat", "w");
   	...
   	for(target1=2; target1<3; target1=target1+1)
   	{
   		...
   		for(target2=L-3; target2<L-2; target2=target2+1)
   		{
  			...
  			double mfp[Totalconfig];
   
   			for(i=0;i<Totalconfig;i++)
   			...
   			for(config=0;config<Totalconfig;config++)
   			{
				...
				int position, position_old;
		   ...
		   
		  		{
		   		...
		   		avgfp=(double)fpsum/ENS;
		   
		  		mfp[config]=avgfp;
		   
		   	}
		   
		   	double mfpsum=0.0; /*sum of mfpt for different configs*/
		   	
		   	double mfpsqsum=0.0; /*sum of square of mfpt for different configs*/
		   
		   	double configavgfp;  /*avg of mfpt for different configs*/
		   	
		   	double configavgsqfp; /*avg of square of mfpt for different configs*/
		   	
		   	double configsd;  /*SD of mfpt for different configs*/
		   
		   	int e;
		   	
		   	double avgdegree[i];
		   
		   	for(e=0;e<Totalconfig;e++)
		   	{
		   
		   		//printf("%lf\n",mfp[e]);
		   	
		   		mfpsum = mfpsum + mfp[e];
		   		mfpsqsum = mfpsqsum + (mfp[e]*mfp[e]);
		   	
		  	}
		   
		   	configavgfp=(double)mfpsum/Totalconfig;

		   	configavgsqfp=(double)mfpsqsum/Totalconfig;
		   	
		   	configsd=pow(configavgsqfp-(configavgfp*configavgfp),0.5);
		   	
		   	
		   	
		   
		   	fprintf(file2, "%lf\t%d\t%lf\t%lf\n", gamma, target2-target1+2, configavgfp, configsd);
			//printf("%lf\t%d\t%lf\t%lf\n", gamma, target2-target1+2, configavgfp, configsd);
